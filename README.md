...
1. **配置环境：** 在 Unity 项目中优先安装必装依赖（Animancer）。
2. **下载导入：** 从 [Releases](../../releases) 下载最新的 `.unitypackage` 并拖入项目。
3. **拼写注意：** 框架核心资源位于 `Assets/Assests` 目录下（请注意拼写中的双 's'）。
4. **运行演示：** 打开 `Scenes` 文件夹下的 Demo 场景，查看 Player 预制体的组件配置。
...
- **特性扩容：** 未来会持续追加更多特性（如持续攀爬/处决等高阶位移机制）、多武器流派模板以及更完善的 AI 驱动源。
